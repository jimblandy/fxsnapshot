...
- [ ] %% Internal structure of streams should be shared, and cloned only when
      we need to produce a value from them. Lazily cloning iterator adapters,
      that hold a `Cow<Rc<I>>` of the underlying iterator, and call `to_mut` to
      clone it when the first value is requested?
